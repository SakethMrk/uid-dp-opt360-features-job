...
public class FeatureBioFaceLivenessScoreDecline extends KeyedProcessFunction<String, InputMessageBio, OutMessage> {

    private static final ObjectMapper objectMapper = new ObjectMapper();
    private static final double ALERT_SLOPE = -1.5;

    public static class FaceScoreState {
        public LinkedList<Double> scores = new LinkedList<>();
        public long lastTimestamp = 0;
        public boolean alerted = false;
    }

    private transient ValueState<FaceScoreState> state;

    @Override
    public void open(Configuration parameters) throws Exception {
        super.open(parameters);
        StateDescriptors.featureBioFaceLivenessScoreDeclineDescriptor.enableTimeToLive(StateDescriptors.stateTtlFunction());
        state = getRuntimeContext().getState(StateDescriptors.featureBioFaceLivenessScoreDeclineDescriptor);
    }
    ...
    public void processElement(InputMessageBio bio, Context ctx, Collector<OutMessage> out) throws Exception {
        ...
        FaceScoreState currentState = state.value();
        if (currentState == null) currentState = new FaceScoreState();

        currentState.scores.addLast(score);
        if (currentState.scores.size() > 8) {
    ...
    private void emitFeature(String key, double slope, FaceScoreState state, Collector<OutMessage> out) {
        OutMessage feature = new OutMessage();
        feature.setOptId(key);
        feature.setFeature("bio_face_liveness_score_decline_v1");
        ...
        comments.put("recent_face_liveness_scores", state.scores);
