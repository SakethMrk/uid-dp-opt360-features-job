...
public class FeatureAuthFaceScoreDecline extends KeyedProcessFunction<String, InputMessageTxn, OutMessage> {
    ...
    public void open(Configuration parameters) throws Exception {
        ...
        StateDescriptors.featureAuthFaceScoreDeclineDescriptor.enableTimeToLive(StateDescriptors.stateTtlFunction());
        state = getRuntimeContext().getState(StateDescriptors.featureAuthFaceScoreDeclineDescriptor);
    }

    @Override
    public void processElement(InputMessageTxn txn, Context ctx, Collector<OutMessage> out) throws Exception {
        String scoreStr = txn.getFaceMatchScore();
    ...
    private void emitFeature(String key, double slope, ScoreState state, Collector<OutMessage> out) {
        ...
        feature.setFeature("auth_face_score_decline_v1");
