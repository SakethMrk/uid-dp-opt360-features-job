...
public class FeatureAuthFingerScoreDecline extends KeyedProcessFunction<String, InputMessageTxn, OutMessage> {
    ...
    public void open(Configuration parameters) throws Exception {
        ...
        StateDescriptors.featureAuthFingerScoreDeclineDescriptor.enableTimeToLive(StateDescriptors.stateTtlFunction());
        state = getRuntimeContext().getState(StateDescriptors.featureAuthFingerScoreDeclineDescriptor);
    }

    @Override
    public void processElement(InputMessageTxn txn, Context ctx, Collector<OutMessage> out) throws Exception {
        String scoreStr = txn.getFingerMatchScore();
        ...
        if (score <= 0) return; // Ignore zero scores if they mean no match performed
        ...
        if (currentState.scores.size() >= 10) {
            ...
            if (slope <= ALERT_SLOPE && !currentState.alerted) {
                ...
                currentState.alerted = false; // reset when it stabilizes
            }
        }
    ...
    private void emitFeature(String key, double slope, ScoreState state, Collector<OutMessage> out) {
        ...
        feature.setFeature("auth_finger_score_decline_v1");
